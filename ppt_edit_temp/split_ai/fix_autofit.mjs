import fs from "node:fs/promises";
import path from "node:path";
import { FileBlob, PresentationFile } from "@oai/artifact-tool";

const root = "G:/mirror_test/ppt_edit_temp/split_ai";
const inputPath = `${root}/final.pptx`;
const outputPath = `${root}/final-v2.pptx`;
const renderDir = `${root}/final-render-v2`;
const layoutDir = `${root}/final-layout-v2/final`;

async function writeBlob(filePath, blob) {
  await fs.mkdir(path.dirname(filePath), { recursive: true });
  await fs.writeFile(filePath, new Uint8Array(await blob.arrayBuffer()));
}

const presentation = await PresentationFile.importPptx(await FileBlob.load(inputPath));
const snapshot = await presentation.inspect({ kind: "textbox", maxChars: 120000 });
const records = snapshot.ndjson.split(/\r?\n/).filter(Boolean).map(JSON.parse);

for (const slideNumber of [6, 7, 8]) {
  const titleRecord = records.find((item) => item.kind === "textbox" && item.slide === slideNumber && String(item.name || "").includes("文本框 2"));
  const bodyRecord = records.find((item) => item.kind === "textbox" && item.slide === slideNumber && String(item.name || "").includes("文本框 1"));
  if (!titleRecord?.id || !bodyRecord?.id) throw new Error(`Missing text boxes on slide ${slideNumber}`);

  const title = presentation.resolve(titleRecord.id);
  title.text.autoFit = "none";
  title.position = { left: 74.81, top: 35.23, width: 1125, height: 95 };

  const body = presentation.resolve(bodyRecord.id);
  body.text.autoFit = "shrinkText";
  body.position = { left: 74.81, top: 166, width: 500, height: 398 };
}

for (const [index, slide] of presentation.slides.items.entries()) {
  const stem = `slide-${String(index + 1).padStart(2, "0")}`;
  await writeBlob(path.join(renderDir, `${stem}.png`), await presentation.export({ slide, format: "png", scale: 1 }));
  const layout = await slide.export({ format: "layout" });
  await fs.mkdir(layoutDir, { recursive: true });
  await fs.writeFile(path.join(layoutDir, `${stem}.layout.json`), await layout.text(), "utf8");
}

await writeBlob(`${root}/final-montage-v2.webp`, await presentation.export({ format: "webp", montage: true, scale: 1 }));
const finalInspect = await presentation.inspect({ kind: "slide,textbox,shape,image,notes,layout", maxChars: 260000 });
await fs.writeFile(`${root}/final-inspect-v2.ndjson`, finalInspect.ndjson || "", "utf8");
const pptx = await PresentationFile.exportPptx(presentation);
await pptx.save(outputPath);
console.log(outputPath);
