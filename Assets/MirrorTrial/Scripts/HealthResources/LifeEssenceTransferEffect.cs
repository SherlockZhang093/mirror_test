...
{
    public sealed class LifeEssenceTransferEffect : MonoBehaviour
    {
        ...
        public static bool TrySpawn(Vector3 origin, Sprite sprite, Color orbColor, float travelDuration)
        {
            if (!sprite)
                return false;

            var root = new GameObject("生命精华传递");
            ...
            var effect = root.AddComponent<LifeEssenceTransferEffect>();
            effect.Configure(renderer, light, sprite, orbColor, travelDuration);
            return true;
        }

        void Configure(SpriteRenderer renderer, Light2D light, Sprite sprite, Color orbColor, float travelDuration)
        {
            ...
            {
                var echo = new GameObject($"生命精华残影 {i + 1}");
        ...
        {
            if (LifeEssenceHudView.TryGetWorldTarget(start, out var worldTarget))
                return worldTarget;
            return start + Vector3.up * 1.5f;
        }
