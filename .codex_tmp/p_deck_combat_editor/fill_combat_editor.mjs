...
const root = "G:/mirror_test/.codex_tmp/p_deck_combat_editor";
...
async function main() {
  ...
  const oldCrop = background.crop;
  const oldFit = background.fit;
  const oldGeometry = background.geometry;
  const oldBorderRadius = background.borderRadius;
  const oldRotation = background.rotation;
  const oldFlipHorizontal = background.flipHorizontal;
  const oldFlipVertical = background.flipVertical;
  const oldLockAspectRatio = background.lockAspectRatio;
  background.replace({
  ...
  background.frame = oldFrame;
  background.crop = oldCrop;
  background.geometry = oldGeometry;
  background.borderRadius = oldBorderRadius;
  background.rotation = oldRotation;
  background.flipHorizontal = oldFlipHorizontal;
  background.flipVertical = oldFlipVertical;
  background.lockAspectRatio = oldLockAspectRatio;
